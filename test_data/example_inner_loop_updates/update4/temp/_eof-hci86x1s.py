...
def score_in_front_of(asset_layout: Layout, target_layout: Layout, min_dist: float, max_dist: float, centered: bool = False) -> float:
    ...
    # REVISION: Increased k_align_y for stricter centering when 'centered' is True
...
def score_near(asset_layout: Layout, target_layout: Layout, max_distance: float, extends_in_front: bool = False) -> float:
    ...
    if extends_in_front:
        # REVISION: Enhanced logic for Area Rug extending in front of Sofa
        
        # Assuming Sofa faces +X (yaw=0) and Rug is also generally aligned.
        # This simplifies the "front" direction to global X.

        # 1. Rug's back edge (min X) should be slightly under sofa's front edge (max X).
        # Ideal: rug.min[0] - sofa.max[0] = -0.15 (15cm under sofa)
        dist_rug_back_to_sofa_front = asset_layout.min[0] - target_layout.max[0]
        k_rug_back = 100.0 # Increased sensitivity for this critical overlap
        score_rug_back = math.exp(-k_rug_back * (dist_rug_back_to_sofa_front - (-0.15))**2)

        # 2. Rug's front edge (max X) should extend significantly beyond sofa's front edge.
        # Ideal: rug.max[0] - sofa.max[0] >= 0.5 (at least 0.5m extension)
        dist_rug_front_to_sofa_front = asset_layout.max[0] - target_layout.max[0]
        k_rug_front = 50.0 # Increased sensitivity
        score_rug_front = math.exp(-k_rug_front * max(0, 0.5 - dist_rug_front_to_sofa_front)**2)

        # 3. Y-alignment: Rug should be horizontally centered with the sofa.
        # Using center locations for Y alignment.
        rug_y_center = asset_layout.location[1]
        sofa_y_center = target_layout.location[1]
        y_center_diff = rug_y_center - sofa_y_center
        k_y_center = 100.0 # Strict centering in Y
        score_y_center = math.exp(-k_y_center * y_center_diff**2)

        # 4. Width Coverage: Rug's width (Y-dimension) should be at least as wide as the sofa.
        rug_width = asset_layout.max[1] - asset_layout.min[1]
        sofa_width = target_layout.max[1] - target_layout.min[1]
        k_width = 50.0 # Penalize if rug is narrower than sofa
        score_width = math.exp(-k_width * max(0, sofa_width - rug_width)**2) 

        return score_dist * score_rug_back * score_rug_front * score_y_center * score_width
...
if __name__ == "__main__":
    ...
    # Increased number of trials for better convergence with stricter constraints.
    num_trials = 2000 
