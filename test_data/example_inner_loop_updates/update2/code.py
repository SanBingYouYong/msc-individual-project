...
def score_near(asset_layout: Layout, target_layout: Layout, max_distance: float, extends_in_front: bool = False) -> float:
    ...
    if extends_in_front:
        ...
        k_rug_back = 50.0
        # Ideal: dist_rug_back_to_sofa_front is slightly negative (rug slightly under sofa)
        # or slightly positive (rug just in front of sofa). Let's aim for -0.1 (10cm under).
        score_rug_back = math.exp(-k_rug_back * (dist_rug_back_to_sofa_front - (-0.1))**2)

        # Score for rug's front extending significantly
        dist_rug_front_to_sofa_front = asset_layout.max[0] - target_layout.max[0]
        k_rug_front = 20.0
        # Penalize if rug's front does not extend beyond sofa's front
        score_rug_front = math.exp(-k_rug_front * max(0, -dist_rug_front_to_sofa_front)**2)
...
def score_on(asset_layout: Layout, target_layout: Layout, draped: bool = False) -> float:
    ...
    if draped:
        ...
        # Let's aim for z_diff = -0.02 (slightly draped, 2cm below target's top)
        k_z = 30.0 # Less strict for draped, but still penalize large deviations
        score_z = math.exp(-k_z * (z_diff - (-0.02))**2)
...
def objective(trial: optuna.Trial, asset_names: List[str], relations_graph: List[Tuple[str, List[str], Dict[str, Any]]]) -> float:
    ...
    for asset_name in asset_names:
        ...
        if asset_name in ["Fireplace", "Sofa", "Coffee Table", "Area Rug"]:
            ...
            loc_z = trial.suggest_float(f"{asset_name}_loc_z", 0.0, 1.0) # Max height of a sofa arm/back
...
if __name__ == "__main__":
    ...
    num_trials = 1000 # Increased trials
