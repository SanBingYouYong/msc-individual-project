...
class Layout:
    ...
    dims: Tuple[float, float, float] # Dimensions of the asset (width_x, depth_y, height_z)
...
def score_against_wall(asset_layout: Layout, wall_plane_normal: Tuple[float, float, float], wall_plane_offset: float, expected_yaw: float = 0.0) -> float:
    ...
    # The asset's front should point into the room.
    # So, its yaw (orientation[2]) should be close to expected_yaw (e.g., pi/2 for facing +Y).
...
def score_in_front_of(asset_layout: Layout, target_layout: Layout, min_dist: float, max_dist: float, centered: bool = False, face_target: bool = False) -> float:
    """
    ...
    vec_x_local = vec_x_global * cos_yaw - vec_y_global * sin_yaw # Distance along target's local X (front)
    vec_y_local = vec_x_global * sin_yaw + vec_y_global * cos_yaw # Distance along target's local Y (side)
    ...
    k_align_y = 150.0 if centered else 10.0 # Stricter alignment if centered
    ...
    expected_asset_yaw = target_yaw + math.pi if face_target else target_yaw
    
    # Normalize yaw difference to be between 0 and pi
    yaw_diff = abs(math.fmod(asset_yaw - expected_asset_yaw + math.pi, 2 * math.pi) - math.pi)
...
def score_near(asset_layout: Layout, target_layout: Layout, max_distance: float, extends_in_front: bool = False) -> float:
    ...
    If extends_in_front is true, it also checks for being in front and covering.
    """
    distance = math.dist(asset_layout.location, target_layout.location)
    
    k_dist = 20.0 # Sensitivity for general proximity
    score_dist = math.exp(-k_dist * max(0, distance - max_distance)**2)

    if extends_in_front:
        # Assume target_layout is Sofa, asset_layout is Area Rug
        target_yaw = target_layout.orientation[2]

        # Calculate relative positions in target's local coordinate system
        vec_x_global = asset_layout.location[0] - target_layout.location[0]
        vec_y_global = asset_layout.location[1] - target_layout.location[1]
        ...
        # Relative position of rug's center in sofa's local space
        rug_center_x_local = vec_x_global * cos_yaw - vec_y_global * sin_yaw # Distance along sofa's forward axis
        rug_center_y_local = vec_x_global * sin_yaw + vec_y_global * cos_yaw # Distance along sofa's side axis

        # Sofa's local dimensions (assuming dims are (width_x, depth_y, height_z) along local axes)
        sofa_local_depth = target_layout.dims[0] # Sofa's depth (front to back)
        sofa_local_width = target_layout.dims[1] # Sofa's width (side to side)

        # Rug's local dimensions
        rug_local_depth = asset_layout.dims[0]
        rug_local_width = asset_layout.dims[1]

        # 1. Rug's center should be slightly in front of sofa's center along sofa's forward axis.
        # This ensures it extends in front.
        # Ideal: rug_center_x_local = (sofa_local_depth / 2.0) + (rug_local_depth / 2.0) - 0.15 (rug's back 15cm under sofa's front)
        # Let's simplify: rug's center should be around 0.5m in front of sofa's center.
        k_rug_x_pos = 50.0
        score_rug_x_pos = math.exp(-k_rug_x_pos * (rug_center_x_local - 0.5)**2)

        # 2. Rug's center should be aligned with sofa's center along sofa's side axis.
        k_rug_y_align = 100.0
        score_rug_y_align = math.exp(-k_rug_y_align * rug_center_y_local**2)

        # 3. Width Coverage: Rug's width should be at least as wide as sofa's width.
        k_width = 50.0
        score_width = math.exp(-k_width * max(0, sofa_local_width - rug_local_width)**2) 

        # 4. Depth Coverage: Rug's depth should be sufficient to extend under and in front.
        # It needs to cover the sofa's depth (partially) and extend forward.
        # Minimum required rug depth: sofa_local_depth (for full coverage) + 0.5 (for extension)
        k_depth = 50.0
        score_depth = math.exp(-k_depth * max(0, (sofa_local_depth + 0.5) - rug_local_depth)**2)

        return score_dist * score_rug_x_pos * score_rug_y_align * score_width * score_depth
...
def objective(trial: optuna.Trial, asset_names: List[str], relations_graph: List[Tuple[str, List[str], Dict[str, Any]]]) -> float:
    ...
    for asset_name in asset_names:
        ...
        # Fireplace needs to be able to place its back at Y=0
        if asset_name == "Fireplace":
            loc_y = trial.suggest_float(f"{asset_name}_loc_y", -0.5, 0.5) # Allow negative Y for fireplace back at Y=0
        else:
            loc_y = trial.suggest_float(f"{asset_name}_loc_y", -1.5, 2.5) # Increased range for other items to be in front of fireplace
        ...
        asset_layouts[asset_name] = Layout(
            ...
            dims=dims # Store dimensions in Layout
        )
    ...
    for relation_type, assets_involved, params in relations_graph:
        ...
        if relation_type == "AGAINST" and len(assets_involved) > 1 and assets_involved[1] == "Wall":
            # Assume wall at Y=0, normal (0, -1, 0) (pointing into room), fireplace faces +Y (yaw=pi/2)
            score = scorer_func(subject_layout, wall_plane_normal=(0.0, -1.0, 0.0), wall_plane_offset=0.0, expected_yaw=math.pi/2)
            total_score += score
        ...
        try:
            if relation_type == "IN_FRONT_OF":
                # Sofa in front of Fireplace: Sofa should face the fireplace (target_yaw + pi)
                # Coffee Table in front of Sofa: Coffee Table should face same direction as sofa (target_yaw)
                face_target_param = False
                if assets_involved[0] == "Sofa" and assets_involved[1] == "Fireplace":
                    face_target_param = True
                
                is_centered = (assets_involved[0] == "Coffee Table" and assets_involved[1] == "Sofa")
                score = scorer_func(subject_layout, target_layout, **params, centered=is_centered, face_target=face_target_param)
...
if __name__ == "__main__":
    ...
    relations_graph = [
        ("AGAINST", ["Fireplace", "Wall"], {"wall_plane_normal": (0.0, -1.0, 0.0), "wall_plane_offset": 0.0}), # Fireplace against wall at Y=0, facing +Y (expected_yaw handled in objective)
        ("IN_FRONT_OF", ["Sofa", "Fireplace"], {"min_dist": 1.5, "max_dist": 2.5}), # Sofa in front of Fireplace (face_target=True, centered=False handled in objective)
        ("IN_FRONT_OF", ["Coffee Table", "Sofa"], {"min_dist": 0.5, "max_dist": 1.0}), # Coffee Table in front of Sofa (face_target=False, centered=True handled in objective)
        ("UNDER", ["Area Rug", "Coffee Table"], {}), # Area Rug under Coffee Table
        ("NEAR", ["Area Rug", "Sofa"], {"max_distance": 1.0}), # Area Rug near Sofa, extending in front (extends_in_front=True handled in objective)
        ("ON", ["Sofa Cushions", "Sofa"], {}), # Sofa Cushions on Sofa
        ("ON", ["Throw Blanket", "Sofa"], {}), # Throw Blanket draped on Sofa (draped=True handled in objective)
    ...
    num_trials = 1000 # Increased trials for better convergence
    ...
    for asset_name, layout in best_asset_layouts.items():
        ...
        f_loc_x, f_loc_y, f_loc_z = layout.location
        f_min_x, f_min_y, f_min_z = layout.min
        f_max_x, f_max_y, f_max_z = layout.max
        f_orient_x, f_orient_y, f_orient_z = layout.orientation
        f_dims_x, f_dims_y, f_dims_z = layout.dims
        print(f"    Location: ({f_loc_x:.2f}, {f_loc_y:.2f}, {f_loc_z:.2f})")
        print(f"    Min BBox: ({f_min_x:.2f}, {f_min_y:.2f}, {f_min_z:.2f})")
        print(f"    Max BBox: ({f_max_x:.2f}, {f_max_y:.2f}, {f_max_z:.2f})")
        print(f"    Orientation (rad): ({f_orient_x:.2f}, {f_orient_y:.2f}, {f_orient_z:.2f})")
        print(f"    Dimensions: ({f_dims_x:.2f}, {f_dims_y:.2f}, {f_dims_z:.2f})")